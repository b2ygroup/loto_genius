...
from firebase_admin import credentials, firestore as admin_firestore # Renomeado para evitar conflito
...
db_admin = None # Cliente Firestore do Firebase Admin
try:
    if not firebase_admin._apps:
        # Tenta primeiro carregar do arquivo local (para desenvolvimento)
        if os.path.exists(SERVICE_ACCOUNT_KEY_PATH_MAIN):
            cred = credentials.Certificate(SERVICE_ACCOUNT_KEY_PATH_MAIN)
            firebase_admin.initialize_app(cred, name='lotoGeniusApiAppMain') # Nome único
            app.logger.info("Firebase Admin SDK inicializado com sucesso para main.py via arquivo.")
        # Senão, tenta carregar de variáveis de ambiente (para Vercel)
        elif os.environ.get('FIREBASE_SERVICE_ACCOUNT_JSON'):
            service_account_json_str = os.environ.get('FIREBASE_SERVICE_ACCOUNT_JSON')
            service_account_info = json.loads(service_account_json_str)
            cred = credentials.Certificate(service_account_info)
            firebase_admin.initialize_app(cred, name='lotoGeniusApiAppMainEnv') # Nome único
            app.logger.info("Firebase Admin SDK inicializado com sucesso para main.py via variável de ambiente.")
        else:
            app.logger.warning("Credenciais do Firebase Admin (arquivo ou ENV VAR) não encontradas. Funcionalidades de admin e leitura de URLs do Firestore não funcionarão.")
    
    if firebase_admin._apps: # Se foi inicializado de alguma forma
        db_admin = admin_firestore.client(app=firebase_admin.get_app(name=list(firebase_admin._apps.keys())[0])) # Pega o app inicializado
        FB_ADMIN_INITIALIZED = True

except Exception as e_fb_admin:
    app.logger.error(f"Erro geral ao inicializar Firebase Admin SDK em main.py: {e_fb_admin}")
...
platform_stats_data = { # ... (como antes) ...
...
def format_currency(value): # ... (como antes) ...
    if isinstance(value, (int, float)): return f"R$ {value:_.2f}".replace('.', ',').replace('_', '.')
    return "R$ 0,00"
def parse_currency_to_float(currency_str): # ... (como antes) ...
...
# --- NOVA FUNÇÃO PARA LER URL DO FIRESTORE E DEPOIS DADOS DO BLOB ---
def load_processed_lottery_data(lottery_key):
    ...
    try:
        ...
        response = requests.get(blob_url, timeout=20) # Timeout aumentado
...
# ... (Resto do seu main.py: get_data_for_stats, endpoints, determinar_faixa_premio_main, verificar_jogos_salvos_batch_main, etc., como estavam na resposta anterior) ...
# Certifique-se que essas funções agora usem `load_processed_lottery_data` que busca do Blob via Firestore.
def get_data_for_stats(lottery_name_lower): #... (como antes, mas a mensagem de erro pode ser ajustada) ...
    ...
    if not all_results:
        return None, {"erro": f"Dados de {lottery_name_lower.upper()} indisponíveis no momento. Tente mais tarde."}, 503
    return all_results, None, None

def combinations_count(n, k): #... (como antes) ...
...
@app.route('/')
def api_base_root(): #... (como antes) ...
    return jsonify({"message": "API Loto Genius Python (api/main.py).", "note": "Endpoints em /api/main/..." })

@app.route('/api/main/')
def api_main_home(): #... (como antes) ...
    return jsonify({"mensagem": "API Loto Genius AI Refatorada!", "versao": "4.3.5 - Blob Integration V2"})

@app.route('/api/main/platform-stats', methods=['GET'])
def get_platform_stats(): #... (como antes) ...
...
def get_recent_winning_pools(): #... (como antes) ...
...
def get_top_winners(): #... (como antes) ...
...
def get_resultados_api(lottery_name): #... (como antes) ...
...
def get_frequencia_numeros(lottery_name): #... (como antes) ...
    lottery_name_lower = lottery_name.lower()
...
def get_pares_frequentes(lottery_name): #... (como antes) ...
...
def get_cidades_premiadas(lottery_name): #... (como antes) ...
    lottery_name_lower = lottery_name.lower()
    all_results, error_response, status_code = get_data_for_stats(lottery_name_lower)
    if error_response: return jsonify(error_response), status_code
...
def get_maiores_premios_cidade(lottery_name): #... (como antes) ...
    lottery_name_lower = lottery_name.lower()
    all_results, error_response, status_code = get_data_for_stats(lottery_name_lower)
    if error_response: return jsonify(error_response), status_code
...
def calcular_probabilidade_jogo(): #... (como antes) ...
    ...
    if lottery_type == "lotomania":
        if len(numeros_usuario) == 50 and num_sorteados_premio_max == 20:
            comb_fav = combinations_count(num_marcados_volante_config, num_sorteados_premio_max) # Correção aqui
...
# --- LÓGICA DE VERIFICAÇÃO DE JOGOS (Adaptada para main.py) ---
def determinar_faixa_premio_main(lottery_type, acertos):
    config_loteria = LOTTERY_CONFIG.get(lottery_type) # Usa o LOTTERY_CONFIG do main.py
...
def verificar_jogos_salvos_batch_main():
    ...
    try:
        ...
        for lottery_key_loop in LOTTERY_CONFIG.keys():
            data = load_processed_lottery_data(lottery_key_loop) # Já lê do Blob
            if data and isinstance(data, list) and len(data) > 0:
                latest_official_results[lottery_key_loop] = data[0]
        if not latest_official_results:
            app.logger.warning("Nenhum resultado oficial carregado para verificação (do Blob). Abortando.")
        ...
        for doc in docs_stream:
            ...
            # Evita re-verificar desnecessariamente se o campo isPremiado já existe E o concurso é o mesmo
            ...
            update_data = {
                ...
                'dataUltimaVerificacao': admin_firestore.SERVER_TIMESTAMP, # Usa timestamp do servidor
                'acertos': hits, 'numerosAcertados': sorted(hit_numbers_list),
                'isPremiado': is_premiado_flag, 'faixaPremio': faixa_premio_str,
                # Define como pendente apenas se se tornou premiado AGORA e não estava antes, ou se estava pendente.
        ...
        msg = f"Verificação em lote concluída. {jogos_atualizados} jogos verificados/atualizados. {jogos_premiados_novos} prêmios marcados para notificação."
...
def trigger_game_verification_endpoint():
    # Substitua "sua-chave-secreta-muito-forte-aqui" por uma chave real
    # E configure INTERNAL_CRON_SECRET como variável de ambiente na Vercel
    INTERNAL_API_KEY = os.environ.get("INTERNAL_CRON_SECRET", "sua-chave-secreta-muito-forte-aqui") 
    ...
    # Aqui, estamos assumindo que os JSONs no Blob já estão atualizados
    # pelo seu script processador_local_loterias.py que roda localmente.
    # Se você quisesse que este endpoint fizesse TUDO (baixar, processar, upload, verificar),
    # a lógica do processador precisaria ser integrada aqui e adaptada para o ambiente serverless.
