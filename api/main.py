...
import pandas as pd
import re
import logging # Para configurar o logger do Flask

app = Flask(__name__)
CORS(app)

# Configurar o logger do Flask para aparecer nos logs da Vercel
app.logger.setLevel(logging.INFO)
# Se rodando localmente e quiser ver no console:
...
def load_processed_lottery_data(lottery_key):
    app.logger.info(f"[load_data] Tentando carregar para {lottery_key.upper()} de {DATA_DIR}")
    config = LOTTERY_CONFIG.get(lottery_key)
    if not config:
        app.logger.error(f"[load_data] Config não encontrada para {lottery_key}")
        return None
    processed_json_path = config.get("processed_json_file")
    if not processed_json_path:
        app.logger.error(f"[load_data] Caminho JSON não configurado para {lottery_key}")
        return None
    if not os.path.exists(processed_json_path):
        app.logger.error(f"[load_data] ARQUIVO JSON NÃO ENCONTRADO: {processed_json_path}")
        return None
    try:
        with open(processed_json_path, 'r', encoding='utf-8') as f: data = json.load(f)
        app.logger.info(f"[load_data] Sucesso ao carregar {lottery_key.upper()} de {processed_json_path}")
        return data
    except Exception as e:
        app.logger.error(f"[load_data] ERRO ao ler/parsear JSON {processed_json_path} para {lottery_key.upper()}: {e}")
        return None
...
def get_data_for_stats(lottery_name_lower):
    ...
    if not all_results:
        app.logger.warning(f"[get_data_for_stats] Dados indisponíveis para {lottery_name_lower} após load_processed_lottery_data.")
        return None, {"erro": f"Dados de {lottery_name_lower.upper()} indisponíveis."}, 404
    app.logger.info(f"[get_data_for_stats] Dados carregados com sucesso para {lottery_name_lower}.")
...
@app.route('/') # Acessível via /api/ se houver uma rota no vercel.json como "src": "/api", "dest": "api/main.py"
def api_base_root():
    app.logger.info(f"Rota / acessada (raiz da função em api/main.py)")
    return jsonify({"message": "API Loto Genius Python (api/main.py).", 
                    "note": "Endpoints principais estão em /api/main/..." })

@app.route('/api/main/')
def api_main_home():
    app.logger.info(f"Rota /api/main/ acessada")
    return jsonify({"mensagem": "API Loto Genius AI Funcionando!", "versao": "3.1.0 - Logs e Rotas Verificadas"})

@app.route('/api/main/platform-stats', methods=['GET'])
def get_platform_stats():
    app.logger.info(f"Endpoint /api/main/platform-stats acessado")
    # ... (sua lógica como antes) ...
    ...
    app.logger.info(f"Retornando platform-stats: {response_data}")
    return jsonify(response_data)

@app.route('/api/main/recent-winning-pools', methods=['GET'])
def get_recent_winning_pools():
    app.logger.info(f"Endpoint /api/main/recent-winning-pools acessado")
    # ... (sua lógica como antes) ...
    pools = [
...
def get_top_winners():
    ...
    app.logger.info(f"Retornando top-winners")
...
def get_resultados_api(lottery_name):
    ...
    all_results = load_processed_lottery_data(lottery_name_lower)
    if not all_results:
        app.logger.warning(f"Resultados não encontrados para {lottery_name_lower} em get_resultados_api")
    ...
    app.logger.info(f"Retornando resultados para {lottery_name}")
    return jsonify(response_data)

# --- Endpoints de Estatísticas ---
# COPIE E COLE AQUI SUAS FUNÇÕES COMPLETAS de estatísticas, garantindo que as rotas comecem com /api/main/
# Exemplo para uma delas:
...
def get_frequencia_numeros(lottery_name):
    ...
    if error_response:
        app.logger.warning(f"Erro ao obter dados para stats/frequencia/{lottery_name}: {error_response}")
        return jsonify(error_response), status_code
    # ... (resto da sua lógica como antes)
    ...
    app.logger.info(f"Retornando stats/frequencia para {lottery_name}")
...
def calcular_probabilidade_jogo():
    ...
    except ValueError: return jsonify({"erro": "Números devem ser inteiros válidos."}), 400
    if len(set(numeros_usuario)) != len(numeros_usuario): return jsonify({"erro": "Números repetidos não permitidos."}), 400
    if len(numeros_usuario) != numeros_marcados_pelo_usuario_no_volante:
           return jsonify({"erro": f"Para {nome_loteria}, você deve fornecer {numeros_marcados_pelo_usuario_no_volante} números."}), 400
...
# --- GERADOR INTELIGENTE (ANTIGO) ---
def gerar_jogo_ia(lottery_name, todos_resultados_historicos, estrategia_req="aleatorio_inteligente", is_premium_user=False):
    app.logger.info(f"[gerar_jogo_ia] Solicitado para {lottery_name}, estratégia {estrategia_req}, premium: {is_premium_user}")
    # ... (COLE SUA LÓGICA COMPLETA AQUI)
    ...
    elif estrategia_req == "equilibrio_par_impar": # ... (e outras estratégias)
    ...
    app.logger.info(f"[gerar_jogo_ia] Jogo gerado para {lottery_name}: {jogo_final} com estratégia {estrategia_aplicada}")
...
def gerar_jogo_api(lottery_name): # ROTA ANTIGA CORRIGIDA
    app.logger.info(f"Endpoint /api/main/gerar_jogo/{lottery_name} acessado com args: {request.args}")
    ...
    if error_resp_stats and lottery_name_lower in LOTTERY_CONFIG:
        app.logger.warning(f"Dados históricos para {lottery_name_lower} não puderam ser carregados para IA em gerar_jogo_api.")
        todos_resultados_para_ia = [] # Permite que a IA funcione com dados vazios se necessário
         ...
         return jsonify(error_resp_stats), 404 # Ou o status_code de error_resp_stats
    ...
    if resultado_geracao.get("erro_premium"):
        app.logger.info(f"Acesso premium negado para gerar_jogo/{lottery_name} com estratégia {estrategia_req}")
    ...
    if not resultado_geracao.get("jogo") or len(resultado_geracao.get("jogo")) == 0 :
        app.logger.error(f"Falha na geração de jogo para {lottery_name} com estratégia {estrategia_req}. Resultado: {resultado_geracao}")
    ...
    app.logger.info(f"Jogo gerado com sucesso para {lottery_name} (estratégia {estrategia_req}).")
...
def gerar_numeros_baseados_em_data_simples(data_nascimento_str, num_numeros, min_val, max_val):
    ...
    while soma_total_digitos > 9: # Redução numerológica básica
    ...
    # Lógica de preenchimento e fallback
    palpite_final = list(numeros_base)
    tentativas_aleatorias = 0
    max_tentativas = num_numeros * 10 # Evitar loops infinitos
    ...
    if len(palpite_final) < num_numeros: # Se ainda faltam números
...
def verificar_historico_combinacao(lottery_name_lower, combinacao_palpite):
    ...
    try: palpite_formatado = sorted([int(n) for n in combinacao_palpite])
        ...
        app.logger.error(f"[verificar_historico] Erro ao formatar palpite: {combinacao_palpite}, Erro: {e}")
        return 0, 0.0
...
def gerar_palpite_esoterico_route(lottery_name):
    ...
    config = LOTTERY_CONFIG.get(lottery_name_lower)
    if not config: 
        app.logger.warning(f"Loteria não configurada para palpite esotérico: {lottery_name}")
        return jsonify({"erro": "Loteria não configurada."}), 404
    
    dados_usuario = request.get_json()
    if not dados_usuario:
        app.logger.warning("Dados da requisição não fornecidos para palpite esotérico.")
        return jsonify({"erro": "Dados da requisição não fornecidos (esperado JSON)."}), 400
    
    data_nascimento_str = dados_usuario.get("data_nascimento")
    if not data_nascimento_str:
        app.logger.warning("Parâmetro 'data_nascimento' não fornecido para palpite esotérico.")
        return jsonify({"erro": "Parâmetro 'data_nascimento' não fornecido."}), 400
    ...
    if not palpite_gerado or len(palpite_gerado) != num_a_gerar:
        app.logger.error(f"Falha ao gerar palpite esotérico com números corretos: {palpite_gerado}, esperado: {num_a_gerar}")
        ...
        app.logger.info(f"Fallback para palpite aleatório: {palpite_gerado}")
...
# # Para ver os logs do app.logger localmente, configure o logging:
#     import logging
#     logging.basicConfig(level=logging.DEBUG) # Use DEBUG para ver mais detalhes
#     # Ou, para formatar como a Vercel (aproximadamente):
#     # formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
#     # ch = logging.StreamHandler()
#     # ch.setFormatter(formatter)
#     # app.logger.handlers.clear() # Limpa handlers padrão do Flask se houver
#     # app.logger.addHandler(ch)
#     # app.logger.propagate = False # Evita duplicidade se root logger também tiver handler

#     app.logger.info(f"Servidor Flask (versão para api/) rodando localmente para teste.")
#     app.logger.info(f"Lendo dados de: {DATA_DIR}")
#     # (código de verificação de JSONs pode ser adicionado aqui)
#     app.run(host='0.0.0.0', port=5000, debug=True) # debug=True ajuda na depuração local